...
class MediumPathFindingDemo extends StatefulWidget {
  const MediumPathFindingDemo({Key? key}) : super(key: key);

  @override
  _MediumPathFindingDemoState createState() => _MediumPathFindingDemoState();
}

class _MediumPathFindingDemoState extends State<MediumPathFindingDemo> {
  ...
  Future<void> runFinder({bool isBFS = false}) async {
    final List<PathElement> queue = <PathElement>[]..add(grid[startRow!][startColumn!]..visited = true);
  ...
  void _addChild({
    ...
    if (row >= 0 && column >= 0 && row < rowSize && column < columnSize) {
      if (!grid[row][column].visited && grid[row][column].passable) {
        final double h = calculateHeuristic(row: row, column: column, isBFS: isBFS);
  ...
  double calculateHeuristic({
    ...
    required bool isBFS,
      ...
      isBFS ? 0 : ((row - goalRow!).abs() + (column - goalColumn!).abs()).toDouble();
...
class PathElement {
  ...
  PathElement? parent; // Marking parent as nullable
